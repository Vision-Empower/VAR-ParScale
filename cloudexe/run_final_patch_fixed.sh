#!/bin/bash
# 执行最终修正版VAR-ParScale真共享骨干测试（修正版）
set -e

SSH_CMD="ssh -p 11292 root@inst-gw.cloudexe.tech"
echo "🚀 执行最终修正版VAR-ParScale真共享骨干测试（修正版）"
echo "="*60

$SSH_CMD << 'EOF'
cd /root/VAR

# 添加补丁到 models/basic_var.py（修正版）
...
def _patch_all_attention_for_batch(model):
    ...
    for blk in model.blocks:
        ...
        # 同时修补 AdaLNSelfAttn 的 forward 方法以支持 cache 参数
        orig_blk_forward = blk.forward
        def blk_fwd(self, x, cache=None):
            if cache is None:
                return orig_blk_forward(x)
            else:
                x = x + self.drop_path(self.attn(self.ln1(x), cache))
                x = x + self.drop_path(self.mlp(self.ln2(x)))
                return x
        blk.forward = types.MethodType(blk_fwd, blk)
PATCH_EOF

# 创建测试脚本（修正版）
cat > test_shared_batch_fixed.py << 'SCRIPT_EOF'
